-- 1. Safely add the images column to your products table if it is missing
ALTER TABLE public.products ADD COLUMN IF NOT EXISTS images TEXT[] DEFAULT '{}';

-- 2. Delete the old bugged RPC to clear the system cache
DROP FUNCTION IF EXISTS public.set_user_role(uuid, text);
DROP FUNCTION IF EXISTS public.set_user_role();

-- 3. Install the bulletproof version
...
-- 4. RELOAD THE SCHEMA CACHE (Fixes the "schema cache" error)
