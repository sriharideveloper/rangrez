...
export async function POST(req) {
  try {
    ...
    if (!secret || !signature) {
      return NextResponse.json({ success: false, error: "Webhook not configured or missing signature" }, { status: 400 });
    }
    ...
    if (body.event === "payment.captured" || body.event === "order.paid") {
      ...
      if (claimError || !sessionData) {
         return NextResponse.json({ success: true, message: "Session already processed by client." });
      }
